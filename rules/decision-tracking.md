...
- **Wiki mode (`decision_tracking.mode: wiki`) does NOT use rolling-window archives.** Each entry is already its own file in `decisions/entries/`; auto-load surface is `decisions/INDEX.md` (a thin index, not the bodies). The rolling-window discipline applies only to single-file mode. Pre-existing quarterly archive files from a single-file→wiki migration (e.g., `decisions/decision-log-YYYY-Q*.md`) are kept as frozen historical artifacts.
