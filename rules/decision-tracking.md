...
- **Append, don't replace.** When the weekly cadence fires twice in the same quarter, the second run appends to the existing `decision-log-YYYY-Q*.md` (don't create `-Q2-second.md`). Quarter rolls over: new file `decision-log-YYYY-Q*.md` with the new quarter.
...
This discipline mirrors the rolling-window pattern in `rules/task-tracking.md`. Decision-log and tasks-status share cadence (weekly), archive shape (`<surface>-YYYY-Q*.md`, meta_layer, archive frontmatter), and trigger logic (cadence OR threshold). The two surfaces drift independently; one may need archive while the other doesn't.
