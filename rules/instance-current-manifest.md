...
## `archives/tasks-status/YYYY-MM.md` (weekly cadence, monthly file)

- **What:** Per-month archive files for completed tasks-status work. Path `archives/tasks-status/YYYY-MM.md` — dedicated folder; one file per calendar month. Classified `meta_layer` (about the instance's history; not propagated to children). Each completed task lands in the current month's file under a dated session block at archive-on-completion time (per v0.9.3+ Pending-only shape).
- **Why it helps:** Keeps the live `tasks-status.md` Pending-only (auto-loaded; must stay small). Monthly file granularity prevents any single archive from clogging the root and stays small enough for fast operator browsing.
- **Required:** No. Instances with thin Done logs can defer creating the folder until the first task completes. Folder convention is canonical; cadence is event-driven (per Pending-only shape).
- **How to add:** When `/acw-session end` Phase 2 archives a completed task, ensure `archives/tasks-status/` exists; write the dated session block to `archives/tasks-status/YYYY-MM.md` for the current month; register the file in `acw-state.yaml::meta_layer` on first write. See `rules/task-tracking.md` § "Done — archive on completion (no interval cadence)."
- **Earned in:** `0.9.0` (initial archive shape); `0.9.5` (moved into `archives/` folder + per-month file granularity; pre-v0.9.5 `tasks-status-YYYY-Q*.md` at workspace root grandfathered).

## `decisions/decision-log-YYYY-Q*.md` archive (single-file mode only)

- **What:** Quarterly archive files for "Decisions and Rationale" entries when the instance runs `decision_tracking.mode: single-file`. Path `decisions/decision-log-YYYY-Q*.md`. Classified `meta_layer`. Open Questions, Constraints and Gotchas, and Resolved Questions do not archive.
- **Why it helps:** `decisions/decision-log.md` is auto-loaded; inline entries cost context every chat. Rolling-window archive keeps the live file lean while preserving full history.
- **Required:** No. Earned when entries age past 7 days OR auto-load cost crosses ~15k tokens.
- **How to add:** When weekly cadence or threshold fires, operator (or `/acw-session end` proposal) creates the archive with archive frontmatter, copies older entries, replaces the archived block with a pointer. Register in `acw-state.yaml::meta_layer`. See `rules/decision-tracking.md` § "Rolling-window discipline."
