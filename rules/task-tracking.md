...
- **Cadence: bi-weekly.** Done session blocks dated more than 14 days old are candidates for archive on the next maintenance pass. Aligns with the same cadence in `rules/decision-tracking.md` so both rolling-window surfaces share one rule shape.
- **Threshold trigger.** If the live file exceeds the ~15k-token ceiling declared in `rules/auto-load-discipline.md` before the bi-weekly cadence fires, archive aggressively regardless of session dates.
- **Archive file pattern.** `tasks-status-YYYY-Q*.md` (e.g. `tasks-status-2026-Q2.md`). Workspace root. Classified `meta_layer` (about this instance's history, not propagated to children). Frontmatter: `class: archive, authority: derived, stability: stable, loaded_by_agent: no`.
- **Pointer line.** Replace archived content in `tasks-status.md` with: *"(Sessions M–N archived to `tasks-status-YYYY-Q*.md`.)"*
- **Build-log narrative** (`build-log.md`) covers the same period in fuller form; the structured-task archive is complementary.
