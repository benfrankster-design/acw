...
- [ ] **v0.10.1 — `/codemap` skill** wrapping Graphify CLI. Verbs: `rebuild`, `rebuild --ast-only`, `rebuild --semantic`, `status`, `audit`. Output routes to `.acw/codemap/GRAPH_REPORT.md` + sidecar JSON. Pre-commit hook integration for AST stage. Requires reading Graphify's actual CLI interface (output format, exit codes, flags) before authoring the wrapper — investigation deferred to dedicated session.
