...
- [ ] **v0.10.1 — Codemap canonical correction sweep (Session 24).** Land the audit fixes from `.acw/raw/2026-05-21-graphify-audit-canonical-vs-reality.md`. 90-min plan documented in session capture `.acw/sessions/2026-05-21--graphify-probe-and-codemap-audit.md` § "The 90-minute patch plan." Includes authoring D-ACW-052 (codemap integration via ACW auto-load, not graphify claude install), C-005 (no Graphify content in CLAUDE.md), and patching `rules/codemap.md`, `rules/confidence-tagging.md`, `rules/instance-current-manifest.md`, `skills/codemap/*`, both migration manifests, scaffolder, and acw-state template.

- [ ] **v0.10.1 — `/codemap` wrapper authoring.** Now ready (Graphify CLI surface documented; ground truth in audit doc). Author `skills/codemap/references/rebuild.md`, `status.md`, `audit.md`. Wrapper does: profile-gate check, run `graphify update <project-root>`, relocate `graphify-out/*` to `.acw/codemap/`, optionally run the `implements_decision` bridge via Claude API. Dogfood against cs-atlas. Est. 1-2 hours.

- [ ] **v0.10.1 — `/acw-instance upgrade` executor verification.** Read the skill's current `skills/acw-instance/references/upgrade.md` and verify the executor handles every step kind the `0.9.9-to-0.10.0.yaml` manifest uses (`remove_gitignore_rule`, `create_dir`, `git_mv` batch, `update_acw_state` with `path_prefix_substrate`/`rename_keys`/`add_fields`/`set_fields`, `update_file` with replace pairs, `rebuild_index`, `only_if` conditional). Patch any gaps. The skill remains a generic executor; it does not know v0.10.0 specifically. Goal: zero skill changes per future version bump — only new manifest files in canonical.
