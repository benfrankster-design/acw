...
- [ ] **v0.10.1 — `/codemap` wrapper authoring (NEXT).** Ready to author. Audit corrections landed in Session 24. Author `skills/codemap/references/rebuild.md`, `status.md`, `audit.md`, plus the pass-through references (`query.md`, `path.md`, `explain.md`). Wrapper does: profile-gate check, CLAUDE.md integrity check per C-005, env_secrets pre-flight (for `--semantic`), run `graphify update <project-root>`, relocate `graphify-out/*` to `.acw/codemap/`, optionally run Stage 2 (Gemini, opt-in), run `implements_decision` bridge via Claude API. Pass-through verbs invoke `graphify query|path|explain` directly. Dogfood against cs-atlas post-v0.10.0 upgrade. Est. 1-2 hours.
